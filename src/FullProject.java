 ...
 * SECOND: Make sure that you have the correct Tables in your database (I sent an emial with all the tables, names, and items)
 * THIRD: Make sure this file is in the correct folder on your computer
 * 
 * NOTE: We probably should have split this up into seprate files but its too late now
 */
public class FullProject {
	static String port = "100.75.220.21"; // What IP you are using
											// (When we connect ot Zoe's computer we will have to change it)
	...
	public static void loginView() {
		...
		while (input != 'q') {
			System.out.println();
			// If register and correct username go to main view
			if (input == 'r') {
				try {
					name = register(scnr);
					mainView(name, scnr);
				} catch (Exception e) {
					System.out.println("ERROR: Username not Vaild\n");
				}
				...
				try {
					...
					System.out.println("ERROR: Credintals not Vaild\n");
				}
				// Anything else
			} else {
	...
	public static String updateAcct(String username, Scanner scnr) {
		...
		try {
			...
			if (!validName(c, newName)) {
				System.out.println("ERROR: Username is Invaild!\n");
				return username;
			}
	...
	public static void createChatroom(String chatroomName, String username, Scanner scnr) {
		...
		try {
			...
			// Insert new chatroom into table
			stmt = c.createStatement();
	...
	public static void readIntput(String chatroomName, String username, String input, int currChat) {
		...
		// IF command run Command methhod
		if (input.charAt(0) == '/') {
		...
		try {
			...
			if ("/list".equals(input)) {
				...
				while (rs.next()) { // checks if rs.next has a value
					String users = rs.getString("username"); // grabs the usernames of everyone logged in the
																// chat room
					System.out.println("USERNAME: " + users);
				}

			} // IF /leave leaves the chatroom
	...
	public static void roomView(String chatroomName, String username, Scanner scnr) {
		...
		/*
		 * I commeted out what the ond roomView was
		 * Most of the commands I copied into the runCommands methods
		 */

		// while (true) {
		// // Change to while view is room
		...
		// while (rs.next()) { // checks if rs.next has a value
		// String users = rs.getString("user"); // grabs the usernames of everyone
		// logged in the
		// // chat room
		// System.out.println("USERNAME: " + users);
		// }
		...
		// String msg = rs.getString("message"); // grabs all the messages from the chat
		// room
